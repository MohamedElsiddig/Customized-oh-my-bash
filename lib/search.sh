...
#———————————————————————————————————————————————————————————————————————————————
# This function returns list of aliases, plugins and completions in oh-my-bash,
# whose name or description matches one of the search terms provided as arguments.
#
# Usage:
#    ❯ oh-my-bash search [-|@]term1 [-|@]term2 ... \
#       [ --enable   | -e ] \
#       [ --disable  | -d ] \
#       [ --no-color | -c ] \
#       [ --refresh  | -r ] \
...
#
# Examples:
#    ❯ oh-my-bash search ruby rbenv rvm gem rake
#          aliases:  bundler
#          plugins:  chruby chruby-auto ruby rbenv rvm ruby
#      completions:  rvm gem rake
#
#    ❯ oh-my-bash search ruby rbenv rvm gem rake -chruby
#          aliases:  bundler
...
#
#    ❯ oh-my-bash search ruby
#          aliases:  bundler
#          plugins:  chruby chruby-auto ruby
#
#    ❯ oh-my-bash search ruby -chruby --enable
#          aliases:  bundler
#          plugins:  ruby
#
# Examples of using exact match:

#    ❯ oh-my-bash search @git @ruby
#          aliases:  git
#          plugins:  git ruby
#      completions:  git
#
...
_OSH-search() {
  _about 'searches for given terms amongst oh-my-bash plugins, aliases and completions'
  _param '1: term1'
  _param '2: [ term2 ]...'
  _example '$ _oh-my-bash-search @git ruby -rvm rake bundler'

  [[ -z "$(type _OSH-array-contains-element 2>/dev/null)" ]] && source "${OSH}/lib/utilities.sh"
  ...
  for word in $@; do
    if [[ ${word} == "--help" || ${word} == "-h" ]]; then
      ...
      export  OSH_SEARCH_USE_COLOR=false
    else
      args=(${args[@]} ${word})
    fi
  done
  ...
  if [[ ${#args} -gt 0 ]]; then
    for component in "${OSH_COMPONENTS[@]}" ; do
      _OSH-search-component "${component}" "${args[@]}"
    done
  fi
...
${echo_underline_yellow}DESCRIPTION${echo_normal}
   ...
   Use ${echo_bold_green}search${echo_normal} oh-my-bash command to search for a list of terms or term negations
...
${echo_underline_yellow}EXAMPLES${echo_normal}
   ...
   For example, ${echo_bold_green}oh-my-bash search git${echo_normal} would match any alias, completion
   ...
   command:
         ...
         ${echo_bold_green}❯ oh-my-bash search git${echo_bold_blue}
   ...
   You can exclude some terms by prefixing a term with a minus, eg:
         ...
         ${echo_bold_green}❯ oh-my-bash search git -flow -svn${echo_bold_blue}
   ...
   that all matches are automatically enabled. The example is below:
         ...
         ${echo_bold_green}❯ oh-my-bash search @git --enable${echo_bold_blue}
               ${echo_bold_yellow}aliases:  ${echo_normal}git
...
${echo_underline_yellow}SUMMARY${echo_normal}
   ...
   Take advantage of the search functionality to discover what oh-my-bash can do
...
_OSH-is-partial-match() {
  local component="$1"
  local term="$2"
  _OSH-component-help "${component}" | $(_OSH-grep) -E -i -q -- "${term}"
}

_OSH-component-term-matches-negation() {
  local match="$1"; shift
  local negative
  for negative in "$@"; do
...
_OSH-search-component() {
  ...
  _example '$ _oh-my-bash-search-component aliases @git rake bundler -chruby'
  ...
  for search_command in "${search_commands[@]}"; do
    if $(_OSH-array-contains-element "--${search_command}" "$@"); then
  ...
  local -a component_list=( $(_OSH-component-list "${component}") )
  local term
  ...
  for term in "${terms[@]}"; do
    ...
    if [[ "${term:0:2}" == "--" ]] ; then
      ...
      if $(_OSH-array-contains-element "${search_term}" "${component_list[@]}"); then
        exact_terms=(${exact_terms[@]} "${search_term}")
      fi
    else
      partial_terms=(${partial_terms[@]} $(_OSH-component-list-matching "${component}" "${term}") )
    fi
  done

  local -a total_matches=( $(_OSH-array-dedup ${exact_terms[@]} ${partial_terms[@]}) )
  ...
  for match in ${total_matches[@]}; do
    ...
    if  [[ ${#negative_terms[@]} -gt 0 ]]; then
      ( _OSH-component-term-matches-negation "${match}" "${negative_terms[@]}" ) && include_match=false
    fi
    ( ${include_match} ) && matches=(${matches[@]} "${match}")
  done
  _OSH-search-result "${component}" "${action}" "${action_func}" "${matches[@]}"
...
_OSH-search-result() {
  ...
  if [[ "${#matches[@]}" -gt 0 ]] ; then
    ...
    for match in "${matches[@]}"; do
      local enabled=0
      ( _OSH-component-item-is-enabled "${component}" "${match}" ) && enabled=1
      ...
      if [[ "${action}" == "${compatible_action}" ]]; then
        if [[ ${action} == "enable" && ${OSH_SEARCH_USE_COLOR} == false ]]; then
          _OSH-flash-term ${len} "${match}${suffix}"
        else
          _OSH-erase-term ${len}
        fi
        modified=1
    ...
    [[ ${modified} -gt 0 ]] && _OSH-clean-component-cache ${component}
    printf "\n"
  fi
}
