cite about-plugin
about-plugin 'Plugin to display reminders when cd-ing into specified directories'

# calls the builtin cd command, and then optionally cat's the .cd-reminder file
# in the directory if it exists
reminder_cd() {
if [[ `declare -F __enhancd::cd` ]]
	then
    		__enhancd::cd "$@" && { [[ ! -a .cd-reminder ]] || cat .cd-reminder 1>&2; }
	else
		cd "$@" && { [[ ! -a .cd-reminder ]] || cat .cd-reminder 1>&2; }
	fi
}
