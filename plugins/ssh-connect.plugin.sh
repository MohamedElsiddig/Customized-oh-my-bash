...
cite about-plugin 
about-plugin 'simple ssh manager. It looks for your previous ssh sessions in $HISTFILE and keeps relevant the ones that you reconnect to.'
listbox() {
  while [[ $# -gt 0 ]]
  do
    key="$1"
...
ssh-connect() {
  ...
  res=$(listbox -t "Connect:" -o "$hist" -a '→' | tee /dev/tty | tail -n 1)
  echo ""
  echo "$res" >> "$HISTFILE"
  eval "$res"
}
