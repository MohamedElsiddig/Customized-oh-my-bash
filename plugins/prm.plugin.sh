#!/usr/bin/env bash
cite about-plugin
about-plugin 'A minimal project manager for the terminal. for more details vist https://github.com/EivindArvesen/prm '
...
function cleanup() {
    ...
    builtin cd "$prm_dir" >/dev/null 2>&1 || return_error 1 "Directory $prm_dir does not exist."
    ...
    builtin cd - >/dev/null 2>&1 || return_error 1 "Previous directory not available."
}
    ...
    case "$1" in
        ...
        stop)
            ...
            if [ -e "$prm_dir/.active-$$.tmp" ]; then
                ...
                builtin cd "$(cat "$prm_dir/.path-$$.tmp")" >/dev/null 2>&1 || return_error 1 "Could not change directory to original path."
